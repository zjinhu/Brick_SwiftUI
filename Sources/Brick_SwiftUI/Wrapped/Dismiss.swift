...
public extension EnvironmentValues {
    ...
    ///
    /// Use this environment value to get the ``Brick.DismissAction`` instance
    /// for the current ``Environment``. Then call the instance
    /// to perform the dismissal. You call the instance directly because
    /// it defines a ``Brick.DismissAction/callAsFunction()``
    /// method that Swift calls when you call the instance.
    ///
    /// For example, you can create a button that calls the ``Brick.DismissAction``:
    ///
    ...
    @_disfavoredOverload
    var dismiss: Brick<Any>.DismissAction {

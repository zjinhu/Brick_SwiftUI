...
public extension ObservableObject {
    ...
    @_disfavoredOverload
    @MainActor
    func withDelaysIfUnsupported(_ keyPath: WritableKeyPath<Self, Brick<Any>.NavigationPath>, transform: (inout Brick<Any>.NavigationPath) -> Void, onCompletion: (() -> Void)? = nil) {
        let start = self[keyPath: keyPath]
        let end = apply(transform, to: start)
    ...
    /// applied in stages.
    @MainActor
    func withDelaysIfUnsupported(_ keyPath: WritableKeyPath<Self, Brick<Any>.NavigationPath>, transform: (inout Brick<Any>.NavigationPath) -> Void) async {
        let start = self[keyPath: keyPath]
        let end = apply(transform, to: start)

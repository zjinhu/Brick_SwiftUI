...
public extension Brick where Wrapped == Any {
    ...
    // An iOS 13+ async/await Brick implementation
    private struct _AsyncImage<Content: View>: View {
        ...
        var content: (Brick<Any>.AsyncImagePhase) -> Content
        ...
        init(url: URL?, scale: CGFloat = 1, transaction: Transaction = Transaction(), @ViewBuilder content: @escaping (Brick<Any>.AsyncImagePhase) -> Content) {
            self.url = url
            self.scale = scale
            self.transaction = transaction
